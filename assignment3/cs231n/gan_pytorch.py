...
def sample_noise(batch_size, dim, seed=None):
    ...
    z = -1 + 2 * torch.rand(batch_size, dim) 
    return z
...
def discriminator_loss(logits_real, logits_fake):
    ...
    sze_r = logits_real.size()
    sze_f = logits_fake.size()
    
    loss = bce_loss(logits_real, torch.ones(sze_r).type(dtype)) + \
           bce_loss(logits_fake, torch.zeros(sze_f).type(dtype))
...
def generator_loss(logits_fake):
    ...
    sze = logits_fake.size()
    loss = bce_loss(logits_fake, torch.ones(sze).type(dtype))
...
def get_optimizer(model):
    ...
    optimizer = optim.Adam(model.parameters(), lr=0.001, betas=(0.5, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
